...
gap-check), and **`guards`** (repo guardrails — plain scripts with stable ids
...
## `guards` — repo guardrails behind a gate

One script per check, with a stable id, fixtures, and exit codes meaning pass,
findings, refused. `.claude/guards.json` names the checks a repo has turned on and
...
[`guards-plugin/README.md`](guards-plugin/README.md).
...
/plugin install guards@no-lost-threads
```
