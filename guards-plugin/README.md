...
| `retro-log` | a line the retro log's grammar cannot read: not a key line (`<class>/<shape>`), an occurrence (`YYYY-MM-DD \| source \| text`), a one-line status (`LANDED\|RETIRED\|UPSTREAM\|FILED\|NOTED\|HELD\|REOPENED <ref> — text`) or annotation (`ADJUDICATED <date> — text`, `ARC <slug> [<date>] — text`), a continuation, or blank; an unknown status token; an arc name that is not a slug; a date after the ref on any token but `ARC`; a status or annotation entry over one line or inside an occurrence; a section after the entries. The grammar is the `threads` plugin's `scripts/retro-log.py`. | `retroLogPath` in `.claude/threads.json`, default `.claude/threads-retro-log.md` |
...
| `plan-sweeps` | a backticked `rg …` in a plan that cannot have run: `rg` exits 2 (a malformed regex, a path it cannot open) or does not finish within 20 seconds; a count the plan states beside a sweep that the sweep does not find; and a sweep piped through `head` or `tail` at any stage, whose output describes the truncation window rather than the population — the message names the remedy (drop it, or state the count off `\| wc -l`) and the span is not run. Each single-line code span beginning `rg ` is split to argv without a shell and run in the checkout; a span the shell would act on (`;`, `\|` other than a trailing `\| wc -l`, `&`, `$`, a redirect, or a glob character outside quotes; `$` or a backtick inside double quotes) and one carrying `--pre`, `--pre-glob`, or `--search-zip` is skipped, never run, and counted on stderr. Exit 0 and 1 are both sane. A number directly after the span — after `→`, `—`, `(`, `:`, `=`, or `returns` — with a unit (`hits`, `matches`, `lines`, `sites` count matching lines; `files` count files; one adjective may sit between; bare or `today` after `→` counts lines; `≥N` / `≤N` are bounds), on the span's line or the next when the span ends its line, is compared with the run's count and a mismatch is a finding naming both numbers. `173:` is an output line and `(4 → 0)` a pair, neither a count. A counting sweep (`-c`, `--count`, `--count-matches`, or `\| wc -l`) followed by a separator alone and a code span holding only an integer (a ledger row's written output) states that integer, compared the same way; a listing's integer span is a line number and is not read. When a claim row states a population — a quantifier or number word (`only`, `exactly`, `all`, `every`, `both`, `one` … `twelve`) in the clause before the sweep or the output's prose, outside parentheses — and its output opens with a file citation, every citation in the rest of the block is read as the sweep's whole output (`File.ext:N`, `N–M`, `,` or `/` lists, `:N` or a bare `N` for a named file, `File.ext` for the whole file, `Stem × N` stating its hits), and a hit none covers is a finding naming it — except a header hit (a line declaring what the sweep matched, a `///` doc comment, a `// MARK:` line), which a caller census rightly leaves out; a witness row with no population word is not compared, nor is a hit in the plan itself or a sweep printing no lines. The stderr summary counts `listings compared` beside `counts compared`. Under a heading beginning with a `targetHeadings` entry (default `Acceptance`) the sweep runs but the count is a target and is not compared. Fenced blocks are not read. `rg` must be on `PATH`. | none; `paths` is required |
...
| `comment-ordinals` | a numbered plan anchor on a leading code-comment line, in three tags and no others (`Slice 4`, `Risk 2`, or a doc named beside `rule 8` is not one): `§` then a digit (`§4`, `§ 4`); `AC` then a digit after at most one space or hyphen, no word character before it (`AC3`, `AC-3`, `AC 3`); `Impl-Note` then a number (`Impl-Note 7`, `Impl. note #7`). A plan renumbers as it is edited and is retired when it ships, so the comment names the symbol or the §"Named section" instead. The comment marker is the file's: `//`, `///`, `/*`, or a block-interior `*` for C-family, Swift, JS/TS, Go, Rust, JVM, C#, and Dart sources; `#` for shell, Python, Ruby, Perl, and awk; for a file with no extension, the marker its shebang's interpreter takes. A trailing comment, a docstring, and a bare letter-and-digit tag (`T4`, a legitimate case id) are not read. A glob in `paths` drops a file whose syntax it does not know, naming on stderr each dropped file with no extension, since `scripts/githooks/*` reads green over a shebang-less file it matched; a literal naming one refuses. | every file with a known source extension; a file with no extension enters only by name, e.g. `scripts/githooks/*` |
...
`scriptDirs`, `commandDirs`, `knownCommands`, and `envPrefixes` there; `plan-sweeps` reads
`targetHeadings`. The runner passes
...
malformed sweep or name an illustrative `scripts/foo.sh` on purpose. In a code file the
marker is the plain text `guards-allow: <id>` in the file's own comment syntax, as
`comment-ordinals` reads it. It is per line and
