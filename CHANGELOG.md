...
- **`ARC <slug> [<date>] — <one line>`**, an annotation beside `ADJUDICATED`: the key rides a named trajectory. Its own entry under the key line; changes neither state nor count; `compact` keeps it on either side of a closing status, so an arc outlives its keys' landings; the `guards` `retro-log` check refuses a name that is not a slug, and a date after the ref on any other token. `retro-log.py view --arc <slug>` prints every key carrying the arc, ordered by the date each entered it (the ARC line's date, else the key's first occurrence; undated last, counted in the header), with its dated occurrences and status lines — the trajectory (attempted, landed, recurred, retired, dated) derived from the log, so a Live ledger watch on an arc cites `view --arc` as its re-derive command and the review stops re-narrating arcs in marker bodies. Stream order alone was wrong on the reference adopter: keys compacted to their `LANDED` line before the arc existed carry no date and sat after the live key they precede, which is why the ARC line takes a date. `view --arcs` lists every arc with its key count and live/closed split; `view --keys` shows `arc:<slug>` on live and closed key lines. The arc's own landings are commits, never keys: a commit that moves an arc carries the trailer `Arc: <slug>`, and **`marker-stream.py list --arc <slug>`** prints those commits dated over the whole history by default, the join the log cannot make (on the reference adopter the deferral arc's four organic commits and its 35 re-formed gates are markers and no key). An arc older than the log's keys gets no anchor-only key; its ledger watch cites `git log --grep` until its next landing carries the trailer. Measured on the reference adopter: one deferral arc spans 22 commits, 3 ledger mentions and 2 log mentions, and the last review judged it in a 737-word body no later run reads; a scope deprecation and its reversal survive only as a parenthetical in a rule; a direction hypothesis sits in two closed `NOTED` entries, two "if the layer recurs" placements, and a stub, with no reader that reassembles them. `test.sh` proves transparency to state and count, survival through `compact` before and after a landing, the slug and dated-status refusals, `--arc` by date entered with undated keys last, `--since` reading an ARC date, `--arcs`, and `marker-stream.py --arc` over the trailer. Adopter-side edit: none required; the review attaches `ARC` to keys already in the log, and an arc is retired by no longer writing the token.
...
## guards [0.12.0] — 2026-09-24

### Added

- **`comment-ordinals`**: a numbered plan anchor — `§4`, `AC3` / `AC-3` / `AC 3`, `Impl-Note 7` — on a leading code-comment line is a finding. The plan renumbers as it is edited and is retired when it ships, so a comment citing its item by number points at nothing a reader of the code can open; the comment names the symbol or the §"Named section", or states the contract. The comment marker is the file's own (`//`, `///`, `/*`, a block-interior `*` for C-family, Swift, JS/TS, Go, Rust, JVM, C#, Dart; `#` for shell, Python, Ruby, Perl, awk; for a file with no extension, the one its shebang's interpreter takes); a trailing comment, a docstring, and a bare letter-and-digit tag (`T4`, a legitimate case id) are not read. The universe without `paths` is every file with a known source extension; a script with no extension enters by name. A glob drops a file it cannot read and names each dropped file with no extension on stderr; a literal naming one refuses. The tag set is these three only — `Slice N`, `Risk N`, and a doc named beside `rule N` are not read. The per-line opt-out is `guards-allow: comment-ordinals` as plain text in the comment. The tags and comment scopes are the reference adopter's lint Check H ordinal arms, verbatim; its `Slice N` / `Risk N` arms and shared-doc ordinals stay local there. Measured on the reference adopter's tree over Check H's scope (`*.swift`, `*.sh`, `scripts/githooks/*`, 608 files): 15 findings, every one a line quoting the tags on purpose under the adopter's own `lint-allow: rule-number` marker, which this check does not honor, so those lines take `guards-allow: comment-ordinals` beside it; no other finding. On this repo: one, a test comment citing a command's section by number. `test.sh` proves each tag spelling, a word carrying the letters, trailing comments and docstrings, the marker, a shebang, a glob dropping an unreadable file where a literal refuses, and an export that drops a source refusing by name. Adopter-side edit: none until a repo enables it; enabling it under an `export` list means exporting the source files it reads.

## guards [0.11.0] — 2026-09-24

### Contract

- **`plan-sweeps` reads a population claim's listing as the sweep's whole output.** A claim row — the claim, then the sweep, then its output — whose output opens with a file citation, and whose claim clause (back to the last `.` or `;`) or output prose carries a quantifier or number word — `only`, `exactly`, `all`, `every`, `none`, `both`, `one` … `twelve`, outside parentheses and hyphenated compounds — has every citation in the rest of its block (list item, table row, or paragraph) read: `File.ext:N`, `N–M` ranges, `,` or `/` lists, the output line's text after it, `Stem:N`, `:N` or a bare `N` for a file the block names, `File.ext` for the whole file, `Stem × N` stating that file's hits. A hit no citation covers is a finding naming the uncited hits at the check's rung, and so is a `Stem × N` whose file holds another number. A header hit need not be cited: a line declaring what the sweep matched (a declaration keyword whose name holds the match, so `let harvest = learner.residualHarvest(` is still a use), a `///` doc comment, or a `// MARK:` line, which a caller census rightly leaves out; a `Stem × N` still counts it. A witness row (`X` exists, then the sweep, then one `F.swift:12`) states no population and is not compared; a hit in the plan carrying the sweep is its own text; a sweep printing no lines and a target heading are skipped. The stderr summary adds `listings compared`. Complements 0.10.0: that reads a count written as a counting row, this a listing written under a population claim. Measured on the reference adopter: replayed over the 43 plans whose drafts cite hits after a sweep, each against the tree it was drafted on, 46 listings compared, 9 findings — four wrong population counts (`two learner tests` over 80 hits, `three Plans/completed/ prose mentions` over six, `seven test callers` over more, `four known hits` over 99), three listings leaving out use sites, and two where the quantifier qualifies something else; its live tree stays green, 39 files, 0 findings. A count recalled with no sweep beside it is out of reach. Adopter-side edit: none.
